...
rt_tol = st.sidebar.number_input("保留时间容差import streamlit as st
import pandas as pd
import numpy as np
...
if non_file and ds_file:
    ...
    # 将 ds 特征转为列表，用于容差匹配
    ds_features = ds_groups[['mz', 'rt', 'name']].values.tolist()  # 每个元素: [mz, rt, name_set]
    ...
    with st.expander("🔎 查看数据样例（用于调试）"):
        ...
        st.write("ds 特征分组示例")
        st.dataframe(ds_groups.head()) (min)", value=0.5, step=0.1, format="%.2f")
